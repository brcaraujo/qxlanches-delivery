...
function displayItems(){
    ...
    const biryaniData= foodItem.filter((item)=>item.category=='hamburguer');
    const chickenData= foodItem.filter((item)=>item.category=='pizza');
    const PaneerData= foodItem.filter((item)=>item.category=='bebidas');
    const vegetableData= foodItem.filter((item)=>item.category=='cerveja');
...
function totalAmount(){
    ...
    // window.localStorage.setItem('valor-total',sum);
    console.log(sum);
    let ajax = new XMLHttpRequest();
    ajax.open('POST', 'https://qxlanches.com.br/api/checkout.php');
    ajax.onreadystatechange=()=>{
        if(ajax.status==200&&ajax.readyState==4){
            // alert(ajax.response);
        }
    }
    ajax.send(JSON.stringify({sum}));
}
